    ...
    var VideoDialog = /** @class */ (function () {
        ...
        VideoDialog.prototype.initialize = function () {
            ...
            var body = [
                ...
                '<p>'+"我的视频"+'</p>',
                '<div style="max-height:300px; overflow: auto;"><ul class="" id="videoList"></ul></div>',
                "<input type='radio' class='videoInputEnabled' id='netVideo' name='videoItem' value='' /><label class=\"note-form-label\">" + this.lang.video.url + " <small class=\"text-muted\">" + this.lang.video.providers + "</small></label>",
                '<input class="note-video-url form-control note-form-control note-input" type="text" id="videoUrl"  / >',
                '</div>'
            ].join('');
            var buttonClass = 'btn btn-primary note-btn note-btn-primary note-video-btn';
            var footer = "<input type=\"button\" href=\"#\" class=\"" + buttonClass + "\" value=\"" + this.lang.video.insert + "\" >";
            this.$dialog = this.ui.dialog({
        ...
        VideoDialog.prototype.createVideoNode = function (url) {
            ...
            //改动
            if(url.indexOf("SteamCoding") != -1){
            	$video = $$1('<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>')
        		.attr('frameborder', 0)
                .attr('src', _URL + "pages/teacherPage/html/videoPlay.jsp?" + url)
        ...
        $(".radioItem").change(function(){
            let url=$("input[name='radioItem']:checked").val();
        });

        VideoDialog.prototype.showVideoDialog = function (text) {
            $.get(postSrc+"queryVideoLink.action",function(result){
            	$("#videoUrl").removeAttr("disabled");
            	$("#netVideo").attr("checked",true);
            	$("#videoList").html("");
                console.log(result);
                for(let i=0;i<result.length;i++){
                    let videoItem='<li><input type="radio" class="videoInputDisabled" name="videoItem" value="'+_URL+result[i].videoLink+'" />'+result[i].videoName+'</li>';
                    $("#videoList").append(videoItem);
                }
                $('input[name="videoItem"]').change(function(){
                    var videoUrl = $("input[name='videoItem']:checked").val();
                    $("#videoUrl").val(videoUrl);
                });

                $('.videoInputDisabled').click(function(){
                    $("#videoUrl").attr("disabled","true");
                });
                $('.videoInputEnabled').click(function(){
                    $("#videoUrl").removeAttr("disabled");
                });
            });
            var _this = this;
