let workOwner = 'others'; //'I'表示我的作品'others'表示其他作品
    ...
    for(let i=0; i<rowNum; ++i){
        ...
        for(let j=0; j<5; ++j){
            ...
            row = row+
                ...
                //'<h5 class="card-title titleStyle" style="text-overflow:ellipsis; overflow:hidden;" title="'+workList[5*i+j]["workName"].split(".")[0]+'">'+ workList[5*i+j]["workName"].split(".")[0] +'</h5>'+
//                '<h5 class="card-title titleStyle" title="'+workList[5*i+j]["createTime"].split(".")[0]+'">'+ workList[5*i+j]["createTime"].split(".")[0] +'</h5>'+
                '<div class="card-text">'+
            ...
            if(workOwner === 'I'){
                ...
                if(isPublished === 1){
                    ...
                    '<div class="col-4 publish-work-btn"><button  type="button" class="btn btn-warning btn-sm btnStyle" id="cancelPublish" data-toggle="modal" data-target="#cancel-publish-modal">撤销</button></div>'+
                    '</div>';
                }
                    ...
                    row = row +  '<div class="card-text row editAndDelete">'+
                        '<div class="col-6 delete-work-btn"><button type="button" data-toggle="modal" data-target="#delete-work-modal" class="btn btn-danger btn-sm" id="deleteScratch">删除作品</button></div>';
                    if(workList[5*i+j].isRecommended === 1){
                        row = row + '<div class="col-6 edit-work-btn"><button type="button" class="btn btn-success btn-sm" id="recommendScratch">取消推荐</button></div>'+
                            '</div>';
                    }
                    else if(workList[5*i+j].isRecommended === 0){
                        row = row + '<div class="col-6 edit-work-btn"><button type="button" class="btn btn-success btn-sm" id="recommendScratch">推荐作品</button></div>'+
                        '</div>';
                    }
                    row = row +  '<div class="card-text row editAndDelete" style="margin-top: 2%">'+
