...
let postSrcStudent=_URL+"student/";
let searchClassName="";
init();
...
/******************添加班级**********************/
$("#addClassCommit").click(function () {
	if($("#addClassName").val() == ""){
		$("#addClassName").addClass("is-invalid");
	}
	if($("#addClassName").hasClass("is-valid")){
		formData = new FormData();
		var name = $("#excel-file").val();
		if($("#excel-file")[0].files[0]){
			formData.append("file",$("#excel-file")[0].files[0]);
			formData.append("name",name);
		}	
		formData.append("className",$("#addClassName").val());
	    $.ajax({
	        url : postSrc+'addClass.action',
	        type : 'POST',
	        async : false,
	        data : formData,
	        processData : false,
...
$("#add-class").click(function(){
	...
	$("#addClassName").removeClass("is-valid");
	$("#addClassName").removeClass("is-invalid");
    $("#fileName").val("未选择文件");
});
//下载导入模板
$("#download-import-template").click(function (e) {
    $.download(postSrcStudent+"downloadTemplate.action","post");
});
...
$("#excel-file").bind("input propertychange",function(event){
	let fileObj = document.getElementById("excel-file").files[0];
	...
	$.post(postSrc+"queryChapterInCourse.action",{classId:getSelectClassId(),courseId:courseId},function (result) {
        ...
        for (let i = 0, index = 1;i<list.length; i++, index++) {
            ...
            if(list[i].isFinished===1){
            	tr.append($("<td><input type='checkbox' name='modalItems' disabled val='"+index+"'></td>"));
            	tr.append($("<td></td>").text(index));
